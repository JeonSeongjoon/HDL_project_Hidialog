...
from args_doc import auto_docstring
from Albert_logging import logging
...
class HiDialog_ALBERT(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        slen = input_ids.size(1)  # sentence length
     
        outputs = self.albert(
            input_ids,
            attention_mask=hidialog_mask, 
            token_type_ids=None,
            position_ids=position_ids,
        ...
        else:  # GTN is not involved
            pooled_output = sequence_outputs[:,0] 
            
        logits = self.classifier(pooled_output)
        logits = logits.view(-1, self.num_labels)

        if labels is not None and self.dataset not in ['MRDA','MuTual']:
            loss_fct = BCEWithLogitsLoss()
            labels = labels.view(-1, self.num_labels)
            loss = loss_fct(logits, labels)
            return loss, logits
        elif labels is not None and self.dataset in ['MRDA']:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits, labels)
            return loss, logits
        elif labels is not None and self.dataset in ['MuTual']:
            logits = logits.view(-1, 4)  # num choices
            loss_fct = CrossEntropyLoss()
            labels = torch.argmax(labels.view(-1, 4), dim=1)  # to locate answer ids
            loss = loss_fct(logits, labels)
            logits = logits.view(-1, self.num_labels)
            return loss, logits         
        else:
...
class GTConv(nn.Module):
    ...
    def forward(self, A):
        ...
        A = torch.sum(A * nn.functional.softmax(self.weight, dim=1), dim=1)
        return A
...
class GTLayer(nn.Module):
    ...
    def forward(self, A, H_=None):
        if self.first:
            ...
            b = self.conv2(A)
            H = torch.bmm(a, b)
            W = [nn.functional.softmax(self.conv1.weight, dim=1).detach(),
                 nn.functional.softmax(self.conv2.weight, dim=1).detach()]
        else:
            a = self.conv1(A)
            H = torch.bmm(H_, a)
            W = [nn.functional.softmax(self.conv1.weight, dim=1).detach()]
...
class GTN(nn.Module):
    def __init__(self, etypes, num_channels, w_in, w_out, num_layers, norm=True, num_class=None):
        ...
        self.layers = nn.ModuleList([
            GTLayer(self.num_edge, num_channels, first=(i == 0)) for i in range(num_layers)
        ])
        self.linear1 = nn.Linear(w_out * num_channels, w_out)
        self.linear2 = nn.Linear(w_out, num_class) if num_class is not None else None
        self.loss = nn.CrossEntropyLoss() if num_class is not None else None
        self.reset_parameters()
    ...
    def norm(self, H, add=False):
        H = H.transpose(1, 2)
        eye = torch.eye(H.size(-1)).to(H.device)
        H = H * (1 - eye) + eye if add else H * (1 - eye)
        deg = torch.sum(H, dim=-1)
        deg_inv = (deg + 1e-8).pow(-1)
        deg_inv[deg == 0] = 0
        deg_inv = deg_inv.unsqueeze(-1) * eye
        H = torch.bmm(deg_inv, H)
        return H.transpose(1, 2)

    def normalization(self, H):
        return torch.stack([self.norm(H[i]) for i in range(self.num_channels)], dim=0)

    def gcn_conv(self, X, H):
        X = torch.matmul(X, self.weight)
        H = self.norm(H, add=True)
        return torch.matmul(H.transpose(1, 2), X)

    def adj(self, graph):
        num_nodes = graph.number_of_nodes('node')
        A = [graph.adj(etype=e).to_dense().float().unsqueeze(-1) for e in self.etypes]
        A.append(torch.eye(num_nodes).unsqueeze(-1).float())
        return torch.cat(A, dim=-1)

    def forward(self, graph, X, target_x=None, target=None, output_gtn_adjacency=False):
        A = self.adj(graph).unsqueeze(0).permute(0, 3, 1, 2)
        Ws = []
        Hs = []
        for i, layer in enumerate(self.layers):
            if i == 0:
                ...
                H, W = layer(A, H)
            Ws.append(W)
            if output_gtn_adjacency:
                Hs.append(H)
        
        X_out = torch.cat([nn.functional.relu(self.gcn_conv(X, H[i])) for i in range(self.num_channels)], dim=1)
        
        if target is not None:
            X_ = self.linear1(X_out)
